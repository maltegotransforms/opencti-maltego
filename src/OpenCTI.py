...
from addEntities import searchAndAddEntity, searchAndAddRelashionship, addRelationship, addReport


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run OpenCTI-Maltego transforms')
    parser.add_argument('--transform', dest='transformName', type=str, help='The transform to run')
    parser.add_argument('--output', dest='output', type=str, default=None, help='To restrict output entity types')
    ...
    if len(unknown) > 0:
        ...
        input_id = client_msg.getProperty("id") if client_msg.getProperty("id") else None
        input_type = client_msg.getProperty("type") if client_msg.getProperty("type") else None
        input_name = client_msg.getProperty("name") if client_msg.getProperty("name") else None
        ...
        if args.transformName.startswith("StixRelation"):
            entity = searchAndAddRelashionship(
                opencti_api_client, transform, input_id
            )
        else:
        ...
        if entity["opencti_entity"]:
            ...
            # *ToReports: Find related reports
                ...
                reports = opencti_api_client.report.list(
                    filters=[{"key": "knowledgeContains", "values": [entity["opencti_entity"]["id"]]}]
                )
            ...
            # StixDomainEntityTo(Relations|StixDomainEntity)(Inferred)?: Return relations|entities when a relation is found
                ...
                if args.output:
                    stix_relations = opencti_api_client.stix_relation.list(
                        fromId=entity["opencti_entity"]["id"], toTypes=[STIX2toOpenCTItype(args.output)], inferred=inferred
                    )
                else:
                    stix_relations = opencti_api_client.stix_relation.list(
                    ...
                    if "Relations" in args.transformName:
                        ...
                        for relation in stix_relations:
                            ...
                            neighbour_entity["maltego_entity"].setLinkLabel(
                                "Inferred\n" if "inferred" in relation and relation["inferred"] else "" +
                                relation["relationship_type"] + "\n" +
                                (relation["first_seen"][0:10] if len(relation["first_seen"]) > 9 else "") + "\n" +
                                (relation["last_seen"][0:10] if len(relation["last_seen"]) > 9 else "")
                            )
            ...
            # StixRelationToStixDomainEntity: Find and add from and to entities
                ...
                if "inferred" in entity["opencti_entity"] and entity["opencti_entity"]["inferred"]:
